import React from "react";
import './Footer.css'
class Footer extends React.Component{
    render(){
        return(
            <div className="footer" style={{height:"72px", width:"100%",backgroundColor:'#f5ebdc',display:"flex",flexWrap:"wrap",justifyContent:"space-around"}}>
                <div className="icons">
                    <div className="icondiv">
                        <img src="data:image/png;base64," alt="" />
                    </div>
                    <div className="textdiv">
                        <p>Home</p>
                    </div>
                </div>
                <div className="icons">
                    <div className="icondiv">
                        <img src="https://www.bk.com/static/media/4f5f9366a38571baec86febd55f0769bc348f9d7-22x22.928445c72424e3a06f12.png" alt="" />
                    </div>
                    <div className="textdiv">
                        <p>Menu</p>
                    </div>
                </div>
                <div className="icons">
                    <div className="icondiv">
                        <img src="data:image/png;base64," alt="" />
                    </div>
                    <div className="textdiv">
                        <p>Offers</p>
                    </div>
                </div>
                <div className="icons">
                    <div className="icondiv">
                        <img src="https://www.bk.com/static/media/daf5c681f2f8aafb1000074e9fa60c327055d59d-22x15.8999aa31fe345e72cf99.png" alt="" />
                    </div>
                    <div className="textdiv">
                        <p>Rewards</p>
                    </div>
                </div>
            </div>
        )
    }
}
export default Footer